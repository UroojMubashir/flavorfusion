...
if (isset($_COOKIE['rememberme'])) {
    ...
    if (mysqli_num_rows($select_users) > 0) {
        ...
        $_SESSION['profile'] = $fetch_user['profile'];
...
<head>
    <meta charset="UTF-8">
    <title>Login - FlavorFusion</title>
                            ...
                            <form action="" method="post" id="login-form">
                                <div class="row g-3 jusify-content-center">
                                    <div class="col-12 mb-1">
                                        <label for="authuser" class="form-label">Email OR Username</label>
                                        <input type="text" name="authuser" id="authuser" required
                                    ...
                                    <input type="hidden" name="redirecturls" value="<?= $redirect_urls ?>"
                                        class="form-control form-input">
                                    <!-- </div> -->
                                    <div class="col-12 mb-0">
                                    ...
                                    <div class="col-12 mb-3 mt-1">
                                        <button class="btn btn-warning shadow-none form-control form-btn p-2"
                                            name="login">Log
                                            in</button>
                                    </div>
    ...
    <script>
        $(document).ready(function () {
            $('#login-form').submit(function (event) {
                ...
                formData.push({ name: 'action-login', value: 'login' });
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (response.status === 'success') {
                            ...
                            showAlert(response.title, 'alert-success bg-success text-white', response.message, 'alert-success bg-success text-white');
                            setTimeout(function () {
                                showAlert('Login successfully.', 'alert-success bg-success text-white', 'Redirecting you...');
                            }, 2000);
                            // after 3 seconds showing the alert message as redirecting
                            setTimeout(() => { window.location.href = response.redirecturl; }, 6000);
                        } else {
                            showAlert(response.title, 'alert-danger bg-danger text-white', response.message, 'alert-success bg-success text-white');
        ...
        function showAlert(alertTitle, alertbg, alertText, removeClasses) {
            var alerts = $('.alert');

            if (removeClasses) {
                alerts.removeClass(removeClasses.split(' '));
            }
